...
gse <- readRDS("PhyBioN_SR_Analysis/gse_PhyBioN_Pseudonymized.RDS")
...
dds <- dds[, c(
  "0Gy-2hrs-do1", "0Gy-2hrs-do2", "0Gy-2hrs-do3", "0Gy-6hrs-do1", "0Gy-6hrs-do2",
  "0Gy-6hrs-do3", "0.5Gy-2hrs-do1", "0.5Gy-2hrs-do2", "0.5Gy-2hrs-do3", "0.5Gy-6hrs-do1",
  "0.5Gy-6hrs-do2", "0.5Gy-6hrs-do3", "1Gy-2hrs-do1", "1Gy-2hrs-do2", "1Gy-2hrs-do3",
  "1Gy-6hrs-do1", "1Gy-6hrs-do2", "1Gy-6hrs-do3", "2Gy-2hrs-do1", "2Gy-2hrs-do2",
  "2Gy-2hrs-do3", "2Gy-6hrs-do1", "2Gy-6hrs-do2", "2Gy-6hrs-do3", "4Gy-2hrs-do1",
  "4Gy-2hrs-do2", "4Gy-2hrs-do3", "4Gy-6hrs-do1", "4Gy-6hrs-do2", "4Gy-6hrs-do3"
)]
...
```{r}
plot_coverage <- plot_totcounts(dds, group = "Dose") + ggpt

ggsave("coverage.png", plot = plot_coverage, width = 11, height = 8, dpi = 400)
...
ggsave("PCA_plot.png", plot = PCA, width = 9, height = 6, dpi = 400)
...
ggsave("Volcano_0Gy.png", plot = volcPlot, width = 8, height = 6, dpi = 400)
```
...
plot_gene_modified <- function(dds_list, gene, intgroup = "condition", assay = "counts",
  ...
  df_list <- lapply(seq_along(dds_list), function(i) {
    ...
    df <- get_expression_values(
      dds = dds, gene = gene, intgroup = intgroup,
...
gene_plot <- plot_gene_modified(
  ...
  gene = c("ENSG00000164430"),
  intgroup = "Dose",
  annotation_obj = anno_df,
  plot_titles = c("2h", "6h")
) + ggpt

ggsave("CGAS.png", plot = gene_plot, width = 8, height = 5, dpi = 400)
```
...
get_cor_test_exp <- function(x) {
  pc <- cor(x$Dose_numeric, x$exp_value, method = "spearman")
  n <- nrow(x)
  c(cor = pc, n = n)
}
...
res_metacors <- lapply(Novel_genes, function(g) {
  ...
  correxp_ID <- vapply(dexp_split, function(x) {
    cor(x$Dose_numeric, x$exp_value, method = "pearson")
  }, numeric(1))
...
goannot <- AnnotationDbi::select(org.Hs.eg.db, keys = keys(org.Hs.eg.db, "SYMBOL"), columns = "GOALL", keytype = "SYMBOL")
