...
<!-- FMsays: use spaces also in the header names, it looks tidyer/in some case it is otherwise failing if no space is in- same goes for some newlines -->

# Exploratory Data Analysis (EDA)

```{r}
## FMsays: always try to NOT use absolute paths but only relative ones. since you are already in the folder where gse.RDS is, go with... (ok, that thing is not there now, but then put it in there)
# gse <- readRDS("~/Phybion/gse.RDS")
gse <- readRDS("gse.RDS")
## FMsays: call this thing gse_phybion_sr?


## FMsays: go with spaces between words and things - one round on this with styler can fix it
# dds<-DESeqDataSet(gse, design= ~Donors+Dose)
dds <- DESeqDataSet(gse, design= ~Donors + Dose)

dds <- dds[,c( "0Gy-2hrs-F"   ,"0Gy-2hrs-A",   "0Gy-2hrs-S",   "0Gy-6hrs-F"   ,"0Gy-6hrs-A",  
...
## FMsays: I am stopping here to change it by hand, go with automated tools
...
## FMsays: you can also have this a bit redundant but also split up by other variables, would not hurt
plot_totcounts(dds, group = "Donors")
plot_totcounts(dds, group = "Time_point")

## FMsays: missing the part where you obtain the vsd object
vsd <- vst(dds)
## FMsays: while at it, call it also vsd_phybion or something more specific?

## PCA plot

pca_plot <- plotPCA(vsd, intgroup = c("Donors", "Time_point"), returnData = TRUE)
## FMsays: need to calculate percentVar somewhere first
...
## FMsays: could be clever to embed these things as covariates in the colData
dds$Time_point
dds$Sex
## FMsays: (and you actually do that)
...
genes<-rownames(mat)
genes <- mapIds(org.Hs.eg.db, keys = genes, column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
rownames(mat)<- genes
...
anno_df <- get_annotation_orgdb(dds = dds,
...
resuSet <- 
  create_DEresults(resuSet =resuSet,
                   dds_obj = dds_t1,
...
# FMsays: we could and should do these operations of refactoring and level processing at the beginning
# FMsays: keep in mind it might change the reference levels
...
res_4Gy_time2_vs_time1 <- results(dds, name = "Time_point6hrs.Dose4Gy", test="Wald")
...
# FMsays: I would probably never rename the rows fully but rather add the symbol in an extra column. Why? you don't want duplicate names or NAs
genes<- rownames(res_shrink_4Gy_time2_vs_time1)
genes <- mapIds(org.Hs.eg.db, keys = genes, column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
rownames(res_shrink_4Gy_time2_vs_time1)<- genes

# FMsays: and indeed you cannot change it
# > rownames(res_shrink_4Gy_time2_vs_time1)<- genes
...
# FMsays: we can probably use mosdef's wrapper here?
...
# FMsays: keep this unevaluated in a separate chunk, so that it does not interrupt the rendering ;)
...
FMsays: feel free to add more text around the chunks. It makes it feel more like you tell what is happening, and that can only be good!

```{r}

# GeneTonic Pathway-associated DEGs Heatmap

gs_heatmap(se = vsd_corrected, 
           gtl = gtl_t1_0.5Gy,
           # FMsays: gtl_5 not existing here so far?
           geneset_id = "GO:0008630", 
           anno_col_info =  c("Dose", "Time_point", "Sex"), 
           # FMsays: go to newlines and let code breathe a bit more
...
# FMsays: we could and probably should use the fgsea implementation here - might need some rework but it might look better and be faster

significant_genes<- resuSet$t2_4Gy_vs_0Gy$tbl_res_all %>%
  as.data.frame() %>%
...
# FMsays: careful, gsea uses ALL genes, not only the signif ones. plus: do not filter post hoc on logFC values, see few lines above
...
p1<- gseaplot(em2, geneSetID = "HALLMARK_TNFA_SIGNALING_VIA_NFKB", 
              by = "runningScore", title = "HALLMARK_TNFA_SIGNALING_VIA_NFKB")

p2 <- gseaplot(em2, geneSetID = "HALLMARK_INFLAMMATORY_RESPONSE", 
...
# FMsays: need this file in the repo too. put the whole folder of gmts?
gs.msdb<- getGmt("c2.cp.v2024.1.Hs.symbols.gmt", geneIdType= GenenameIdentifier(), collectionType = BroadCollection("c2"))
...
corfit<-duplicateCorrelation(gsva_matrix,design,block=patient)

fit <- lmFit(gsva_matrix,design,block=patient,correlation=corfit$consensus)
...
results_gsva<-topTable(fit2,coef=1,number=nrow(gsva_matrix), adjust="BH")
...
decoupler_genes<-mapIds(org.Hs.eg.db, keys = decoupler_genes, column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
rownames(decoupler_counts)<- decoupler_genes

# FMsays: split up a bit this very long chunk and tell what happens

res_t1_0.5Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t1_0.5Gy_vs_0Gy)<- res_t1_0.5Gy_vs_0Gy$SYMBOL

res_t1_1Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t1_1Gy_vs_0Gy)<- res_t1_1Gy_vs_0Gy$SYMBOL

res_t1_2Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t1_2Gy_vs_0Gy)<- res_t1_2Gy_vs_0Gy$SYMBOL

res_t1_4Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t1_4Gy_vs_0Gy)<- res_t1_4Gy_vs_0Gy$SYMBOL

res_t2_0.5Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t2_0.5Gy_vs_0Gy)<- res_t2_0.5Gy_vs_0Gy$SYMBOL

res_t2_1Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t2_1Gy_vs_0Gy)<- res_t2_1Gy_vs_0Gy$SYMBOL

res_t2_2Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)

rownames(res_t2_2Gy_vs_0Gy)<- res_t2_2Gy_vs_0Gy$SYMBOL

res_t2_4Gy_vs_0Gy<- mosdef::deresult_to_df(resuSet$t1_0.5Gy_vs_0Gy$res_DESeq, FDR = 0.05)
...
sample_acts_t1_0.5Gy <- run_ulm(mat=res_t1_0.5Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')

sample_acts_t1_1Gy <- run_ulm(mat=res_t1_1Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t1_2Gy <- run_ulm(mat=res_t1_2Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t1_4Gy <- run_ulm(mat=res_t1_4Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t2_0.5Gy <- run_ulm(mat=res_t2_0.5Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t2_1Gy <- run_ulm(mat=res_t2_1Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t2_2Gy <- run_ulm(mat=res_t2_2Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')


sample_acts_t2_4Gy <- run_ulm(mat=res_t2_4Gy_vs_0Gy[, 'stat', drop = FALSE], net=net, .source='source', .target='target', .mor='mor')

# FMsays: errors here, maybe because of the missing rowname operation?
...
sample_acts <- run_ulm(mat=decoupler_counts, net=net, .source='source', .target='target',.mor='mor', minsize = 5)
...
# Plot
pheatmap(t(sample_acts_mat), border_color = NA, color=my_color, breaks = my_breaks, annotation_col = column_annotation,cluster_cols = TRUE) 
...
# FMsays: name here, racle is one example dataset :) plus you need the symbols?
...
pheatmap(t(ti_racle_mat_ordered[,-1])- rowMeans(t(ti_racle_mat_ordered[,-1])), cluster_cols = FALSE)
...
#Omnideconv (in Docker) and CIBERSORTx (in web-based tool) were also performed

```

FMsays: as an additional section, we can add something like a recap one
And also: something where the tabular objects in R are then later exported, ideally all in something like the `_output` folder, with names clevely chosen for them?
