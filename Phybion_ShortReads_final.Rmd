...
topVarGenes <- head(order(rowVars(assay(vsd_corrected)), decreasing = TRUE), 1000)
...
cluster_mosdef <- run_cluPro(
  de_container = dds,
  res_de = resuSet$t1_2Gy_vs_0Gy$res_DESeq,
  mapping = "org.Hs.eg.db",
  keyType = "SYMBOL",
  ont = "BP", de_genes= resuSet$t1_2Gy_vs_0Gy$tbl_res_DE$SYMBOL , FDR_threshold = 0.05,bg_genes = geneUniverseExpr)


cluster_mosdef<-cluster_mosdef@result[cluster_mosdef@result$p.adjust<=0.05,]
View(cluster_mosdef)

cluster_origin<-resuSet$t1_2Gy_vs_0Gy$clupro_tbl@result[resuSet$t1_2Gy_vs_0Gy$clupro_tbl@result$p.adjust<=0.05,]
...
vsds_vars<- rowVars(decoupler_counts)
...
decoupler_counts$Vars<- vsds_vars

bulk_mat_dup<-  bulk_mat$SYMBOL[duplicated(bulk_mat$SYMBOL)]

bulk_mat_dup<- bulk_mat[bulk_mat$SYMBOL %in% bulk_mat_dup, c("SYMBOL", "id")]

#res_duplicates <- res_t1_0.5Gy_vs_0Gy$SYMBOL[duplicated(res_t1_0.5Gy_vs_0Gy$SYMBOL)]

#res_duplicates<- res_t1_0.5Gy_vs_0Gy[res_t1_0.5Gy_vs_0Gy$SYMBOL %in% res_duplicates, c("SYMBOL", "id")]

dds_mat_dup<- bulk_mat[bulk_mat_dup$id, ]


dds_mat_dup <- cbind (dds_mat_dup, bulk_mat_dup$SYMBOL)

dds_mat_dup<- as.data.frame(dds_mat_dup)


dds_mat_dup <- dds_mat_dup %>% rownames_to_column("Ensembl_ID")


filtered_ddd <- dds_mat_dup %>%
  group_by(SYMBOL) %>%
...
dds_mat_dup <- dds_mat_dup %>% column_to_rownames("Ensembl_ID")


ddd_low_var<- as.data.frame(dds_mat_dup[ ! rownames(dds_mat_dup)  %in%  rownames(filtered_ddd), ])

bulk_mat <- bulk_mat[!rownames(bulk_mat) %in% rownames(ddd_low_var), ]
...
bulk_mat<- bulk_mat[ rowSums(bulk_mat)>0 , ]
...
bulk_mat<- cbind(bulk_mat, SYMBOL=bulk_genes)
bulk_mat<- as.data.frame(bulk_mat)
bulk_mat$id<- rownames(bulk_mat)

bulk_mat <- bulk_mat[!rownames(bulk_mat) %in% rownames(ddd_low_var), ]

bulk_mat<- na.omit(bulk_mat)

bulk_mat <- bulk_mat[!apply(bulk_mat == "", 1, any), ]

rownames(bulk_mat)<- bulk_mat$SYMBOL
...
degs_t1<- c(resuSet$t1_0.5Gy_vs_0Gy$tbl_res_DE$id, resuSet$t1_1Gy_vs_0Gy$tbl_res_DE$id,resuSet$t1_2Gy_vs_0Gy$tbl_res_DE$id,resuSet$t1_4Gy_vs_0Gy$tbl_res_DE$id)

degs_t1<- unique(degs_t1)

degs_t2<- c(resuSet$t2_0.5Gy_vs_0Gy$tbl_res_DE$id, resuSet$t2_1Gy_vs_0Gy$tbl_res_DE$id,resuSet$t2_2Gy_vs_0Gy$tbl_res_DE$id,resuSet$t2_4Gy_vs_0Gy$tbl_res_DE$id)

degs_t2<- unique(degs_t2)

assay_to_use<- assay(vsd_t1_corrected)

#topVarGenes <- head(order(rowVars(assay(vsd_corrected)), decreasing = TRUE), 100)

assay_to_use_de <- assay_to_use[degs_t1,]
assay_to_use_de <- assay_to_use_de - rowMeans(assay_to_use_de)


set.seed(2024)

ha3 <- Heatmap(assay_to_use_de, name = "Z-score\nexpression\nvalues", 
               col = circlize::colorRamp2(c(-max(abs(assay_to_use_de)), 
                                           0, 
                                           max(abs(assay_to_use_de))), 
                                          c("navy", "floralwhite", "firebrick")), 
               row_km = 8,
               show_row_names = TRUE,
...
rownames(assay_to_use_de)

de_grouped_rows <- row_order(ha3)
de_grouped <- lapply(de_grouped_rows, function(arg) {
  this_cluster <- arg
  genes_cluster <- rownames(assay_to_use_de)[this_cluster]
  genes_cluster <- genes_cluster[!is.na(genes_cluster)]
  genes_cluster <- sort(genes_cluster)
  genes_cluster
})

names(de_grouped) <- paste0("DE cluster ", 1:8)
